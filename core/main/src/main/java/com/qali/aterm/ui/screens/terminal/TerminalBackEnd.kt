...
import com.qali.aterm.ui.screens.terminal.virtualkeys.SpecialButton
...
class TerminalBackEnd(val terminal: TerminalView,val activity: MainActivity) : TerminalViewClient, TerminalSessionClient {
    ...
    override fun onKeyDown(keyCode: Int, e: KeyEvent, session: TerminalSession): Boolean {
        if (keyCode == KeyEvent.KEYCODE_ENTER && !session.isRunning) {
            ...
            if (activity.sessionBinder!!.getService().getVisibleSessions().isEmpty()){
                activity.finish()
            }else{
                changeSession(activity,activity.sessionBinder!!.getService().getVisibleSessions().first())
            }
            return true
        }
        return false
